...
| <a name="input_access_log_prefix"></a> [access\_log\_prefix](#input\_access\_log\_prefix) | Key prefix for the access logs. Only read when access\_log\_bucket is set. Ends with a slash, or the prefix runs into the object name. | `string` | `"s3-access-logs/"` | no |
